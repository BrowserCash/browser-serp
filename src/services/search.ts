...
export async function runGoogleSearch(
  ...
  let { results, html: lastHtml, stepTimings } = await fetchAndParse(baseUrl, "google-search");
  ...
  while (results.length < count && pageNum < MAX_PAGES) {
    ...
    try {
      ...
      if (await nextBtn.isVisible({ timeout: 400 })) {
        if (DEBUG_LOG) log(`[serp] clicking next page (current: ${results.length}, target: ${count})`);
        await nextBtn.click();
